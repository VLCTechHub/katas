...
const char *fullSong = "99 bottles of beer on the wall, 99 bottles of beer. Take one down and pass it around, 98 bottles of beer on the wall.\n\
\n\
...
describe("99 bottles", []() {
  it("generates verse for the number of bottles, and new number of bottles", []() {
    int numBottles = 99;

    auto result = Bottles_BuildVerse(numBottles);

    expect(result.verse).toEqual("99 bottles of beer on the wall, 99 bottles of beer. Take one down and pass it around, 98 bottles of beer on the wall.");
    expect(result.remainingBottles).toEqual(98);
  });

  it("takes into account when 2 bottles are left", []() {
    int numBottles = 2;

    auto result = Bottles_BuildVerse(numBottles);

    expect(result.verse).toEqual("2 bottles of beer on the wall, 2 bottles of beer. Take one down and pass it around, 1 bottle of beer on the wall.");
    expect(result.remainingBottles).toEqual(1);
  });

  it("takes into account when 1 bottles is left", []() {
    int numBottles = 1;

    auto result = Bottles_BuildVerse(numBottles);

    expect(result.verse).toEqual("1 bottle of beer on the wall, 1 bottle of beer. Take it down and pass it around, no more bottles of beer on the wall.");
    expect(result.remainingBottles).toEqual(0);
  });

  it("takes into account when 0 bottles are left", []() {
    int numBottles = 0;

    auto result = Bottles_BuildVerse(numBottles);

    expect(result.verse).toEqual("No more bottles of beer on the wall, no more bottles of beer. Go to the store and buy some more, 99 bottles of beer on the wall.");
    expect(result.remainingBottles).toEqual(-1);
  });
  ...
  it("sings full song 🎸🎸🎶🎶", []() {
    ...
    auto song = Bottles_Sing(numBottles);

    expect(song).toEqual(fullSong);
  });
});
